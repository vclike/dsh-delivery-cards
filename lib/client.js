...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		function reportDecision(raw, passed) {
			if (decisionReported) return;
			decisionReported = true;
			probe({ stage: "boot", raw, passed, accepted: passed > 0 });
		}
		...
		function selectCards(owner) {
			try {
				const raw = owner?.turn?.data?.get?.("deliverables")?.presented;
				const files = presentedFor(owner);
				reportDecision(Array.isArray(raw) ? raw.length : -1, files.length);
