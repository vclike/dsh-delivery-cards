...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 本 bundle 的构建标记：探针日志里能看到它，就能确认浏览器加载的是新代码。 */
		const BUILD = "v3-probe";

		/** 诊断探针路由（宿主半边登记）；失败静默，绝不影响主流程。 */
		const PROBE_PATH = "/api/dsh-delivery-cards.probe";

		/**
		 * 向宿主探针上报一条诊断。**必须静默**——诊断代码不能干扰被测行为。
		...
		function DeliveryCards(props) {
			...
			const run = (file, action) => {
				...
				probe({ stage: "click", action, url });
				fetch(url, { method: "POST" })
					.then((response) => {
						probe({ stage: "result", action, status: response.status, ok: response.ok });
						setPhase(
							response.ok ? `ok:${action}` : `error:${response.status}`,
						);
					})
					.catch((error) => {
						probe({ stage: "throw", action, message: String(error) });
						setPhase("error:0");
					});
			};
